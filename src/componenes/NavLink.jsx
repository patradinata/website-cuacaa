  ...
  return (
    <nav className={`fixed z-50 top-0 left-0 bg-white w-64 h-screen px-6 pt-4 transform transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <ul className="mt-6 flex flex-col gap-4">
        {links.map((link) => (
          <li key={link.name} className={`rounded-md px-4 transition-all duration-300 ${activeLink === link.href ? "bg-indigo-300 scale-105" : "hover:bg-slate-100"}`}>
            <Link
              to={link.href}
              className={`block py-2 text-sm font-medium capitalize text-indigo-800 transition-transform duration-300 ${activeLink === link.href ? "font-semibold" : ""}`}
      ...
      <div className="mt-12 z-50 w-full">
        <Link to={"/Login"} onClick={() => setOpen(!open)} className="bg-indigo-500 hover:bg-indigo-700 active:scale-95 text-base px-6 py-2 font-bold text-white rounded-xl md:hidden">
          Login
        </Link>
      </div>
    </nav>
