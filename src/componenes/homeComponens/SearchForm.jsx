  ...
  useEffect(() => {
    ...
    if (formRef.current) {
      gsap.fromTo(
        ...
        { y: 0, opacity: 1, duration: 3, ease: "power3.out" } // Bergerak ke bawah dengan opacity 1
      );
    }
  }, []);
  ...
  return (
    <form ref={formRef} onSubmit={handleSearch} className="w-full max-w-2xl md:px-4 px-12">
      <div className="relative">
        <input
          type="text"
          placeholder="search name city here..."
          className="w-full py-3 px-8 rounded-full bg-white bg-opacity-80 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-blue-300 transition duration-300"
          value={searchQuery}
