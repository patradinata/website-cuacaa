  ...
  useEffect(() => {
    if (mainCityRef.current) {
      gsap.fromTo(
        ...
        { x: 0, opacity: 1, duration: 3, ease: "power3.out", delay: 0.5 } // Animasi bergerak masuk ke kanan dengan delay
      );
    }
  }, []);

  if (!city) return null; // Jika city tidak ada, maka tidak akan menampilkan komponen ini

  return (
    <Link ref={mainCityRef} to={`/city/${city.name}`} className="bg-white bg-opacity-80 backdrop-blur-5xl p-6 rounded-lg shadow-lg max-w-sm w-full hover:bg-indigo-200 transition-colors mt-6 mb:mt-12 lg:mt-0">
