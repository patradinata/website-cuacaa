...
function CityDetail() {
  ...
  useEffect(() => {
    ...
    if (cityDetail && cardRef.current) {
      ...
      ctx = gsap.context(() => {
        // Initial state
        gsap.set([cardRef.current, titleRef.current, timeRef.current, weatherDisplayRef.current, detailsRef.current, alertRef.current, buttonRef.current], {
          opacity: 0,
          y: 20,
        });
    ...
    <div className="min-h-screen bg-black relative">
      ...
      <div ref={containerRef} className="container mx-auto p-6 relative pt-32">
        <div ref={cardRef} className="opacity-0 bg-white bg-opacity-80 backdrop-blur-xl p-8 rounded-lg shadow-lg max-w-4xl mx-auto text-center">
          ...
          <Link ref={buttonRef} to="/" className="inline-block mt-9 w-72 bg-indigo-600 text-white py-2 px-7 rounded-lg hover:bg-indigo-700 active:scale-50 transition duration-300">
            Kembali
          </Link>
        </div>
      </div>
