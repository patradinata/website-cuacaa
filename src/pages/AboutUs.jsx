...
const AboutUs = () => {
  ...
  useEffect(() => {
    ...
    tl.to(cardRef.current, {
      ...
      .to(
        [cloudRef.current, sunRef.current, sparklesRef.current],
        {
          duration: 0.8,
          scale: 1,
          opacity: 1,
  ...
  return (
    <div className="min-h-screen pt-20 pb-36 flex items-center justify-center bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-6 overflow-hidden">
      <div ref={cardRef} className="relative bg-white/90 backdrop-blur-xl p-12 rounded-2xl shadow-2xl w-full max-w-4xl transform hover:scale-[1.02] transition-transform duration-300" style={{ opacity: 0, transform: "translateY(100px)" }}>
        {/* Decorative Elements */}
        ...
        <h2 ref={titleRef} className="text-4xl font-bold text-center bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent mb-8" style={{ opacity: 0, transform: "translateY(-50px)" }}>
          About Us
        </h2>

        <div ref={contentRef} className="space-y-6 text-center font-medium`">
        ...
        <div className="mt-8 text-center">
          <button className="px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-full font-semibold hover:shadow-lg transform hover:-translate-y-1 transition-all duration-300">Contact Us</button>
        </div>
      </div>
    </div>
  );
