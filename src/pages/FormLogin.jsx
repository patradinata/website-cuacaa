...
const FormLogin = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-cyan-500 to-indigo-500  p-6 relative overflow-hidden">
      {/* Animated background elements */}
      <div className="absolute w-96 h-96 bg-pink-400/30 rounded-full blur-3xl -top-10 -right-10 animate-pulse"></div>
      <div className="absolute w-96 h-96 bg-blue-400/30 rounded-full blur-3xl -bottom-10 -left-10 animate-bounce"></div>

      <div ref={formRef} className="relative bg-white/10 backdrop-blur-lg p-8 rounded-2xl shadow-2xl w-full max-w-md border border-white/20" style={{ opacity: 0, transform: "translateY(20px)" }}>
        <div ref={titleRef} className="text-center mb-8">
          <h2 className="text-3xl font-bold bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent">Welcome Back</h2>
          <p className="text-white/60 mt-2">Please sign in to continue</p>
        </div>

        {error && <div className="bg-red-500/10 border border-red-500/20 text-red-300 text-sm py-2 px-4 rounded-lg mb-4">{error}</div>}
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div ref={buttonRef} className="flex items-center justify-between">
            ...
            <a href="#" className="text-sm text-white/60 hover:text-white transition-colors underline">
              Forgot Password?
            </a>
          </div>
        </form>

        <div ref={linksRef} className="text-sm text-center text-white/100 mt-8">
          Don't have an account?{" "}
          <a href="#" className="text-white/100 hover:text-blue-300 transition-colors mx-2">
            Sign up here
          </a>
        </div>
      </div>
